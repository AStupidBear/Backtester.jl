function 产生目录(时间戳, 代码, 年化收益率, 最大回撤, 夏普率)
    ...
    开始时间戳 = minimum(t -> ifelse(iszero(t), Inf, t), 时间戳)
    结束时间戳 = maximum(t -> ifelse(iszero(t), -Inf, t), 时间戳)
...
combine(dirs; remove = false) = (res = 合并汇总(dirs); remove && rm.(dirs, force = true, recursive = true); res)
