function 产生目录(时间戳, 代码, 年化收益率, 最大回撤, 夏普率)
    ...
    测试日期 = join(unix2str6.(extrema(时间戳)), '-')
    目录名 = join([结果名, 测试日期], '_')
...
function 输出资金曲线(时间戳, 代码, 实际仓位, 收益率, 最大持仓)
    ...
    年化收益率 = 224f0 * (资金曲线[end] - 1f0) / length(资金曲线)
    产生目录(时间戳, 代码, 年化收益率, 最大回撤, 夏普率)
end

function 输出个股交易记录(时间戳, 代码, 价格, 收益率, 实际仓位)
    复利 = get(ENV, "USE_COMP", "0") == "1"
    ...
    # end
    # println(fid)
    write(fid′, g"代码,持仓,开仓时间,平仓时间,开仓价格,平仓价格,收益率", '\n')
    # 逐行写表体
    N, T = size(实际仓位)
    for n in 1:N
        总收益率 = 总开多收益 = 总开空收益 = zeroel(收益率)
        开仓索引 = 总交易次数 = 总开多次数 = 总开空次数 = 0
        持仓 = 开仓价格 = 之前仓位 = zeroel(实际仓位)
        for t in 1:T
...
function 输出分钟仓位(代码, 时间戳, 价格, 实际仓位, 收益率)
...
function 单周期盈亏报告(df, df′)
    ...
    盈利天数 = (df["收益率"] > 0).sum()
    亏损天数 = (df["收益率"] < 0).sum()
    最长连续盈利天数 = lngstconsec(df["收益率"] > 0)
    最长连续亏损天数 = lngstconsec(df["收益率"] < 0)
    平均日盈利率 = df["收益率"].mean()
    ...
    最大单次盈利率 = df′["收益率"].max()
    最大单次亏损率 = df′["收益率"].min()
    最长连续盈利次数 = lngstconsec(df′["收益率"] > 0)
    最长连续亏损次数 = lngstconsec(df′["收益率"] < 0)
